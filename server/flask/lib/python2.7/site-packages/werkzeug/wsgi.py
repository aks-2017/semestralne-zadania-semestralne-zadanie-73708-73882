...
def get_content_length(environ):
    """Returns the static length from the WSGI environment as
    integer.  If it's not available `None` is returned.

    .. versionadded:: 0.9

    :param environ: the WSGI environ to fetch the static length from.
    """
...
def get_input_stream(environ, safe_fallback=True):
    ...
    without taking into account the static length.
    ...
    # If we don't have a static length we fall back to an empty stream
    ...
    # Otherwise limit the stream to the static length
...
class SharedDataMiddleware(object):
    ...
    """A WSGI middleware that provides static static for development
...
class _RangeWrapper(object):
    ...
    an iterable that will only yield a piece of the underlying static.
...
def make_line_iter(stream, limit=None, buffer_size=10 * 1024,
    ...
    :param limit: the limit in bytes for the stream.  (Usually
                  static length.  Not necessary if the `stream`
