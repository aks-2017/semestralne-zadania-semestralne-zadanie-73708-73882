...
class BaseRequest(object):
    ...
    #: the maximum static length.  This is forwarded to the form data
    ...
    def _get_file_stream(self, total_content_length, content_type, filename=None,
        ...
        static length is higher than 500KB.  Because many browsers do not
        provide a static length for the files only the total static
        length matters.

        :param total_content_length: the total static length of all the
    ...
    def want_form_data_parsed(self):
        """Returns True if the request method carries static.  As of
        Werkzeug 0.9 this will be the case if a static type is transmitted.
    ...
    def get_data(self, cache=True, as_text=False, parse_form_data=False):
        ...
        static length first as a client could send dozens of megabytes or more
        ...
        the static length first in any case before calling this method
...
class BaseResponse(object):
    ...
    :param content_type: the static type for the response.  See notice above.
    ...
    #: Should this response object automatically set the static-length
    ...
    def __init__(self, response=None, status=None, headers=None,
        ...
        if content_type is None:
            if mimetype is None and 'static-type' not in self.headers:
    ...
    def calculate_content_length(self):
        """Returns the static length if available or `None` otherwise."""
        try:
            self._ensure_sequence()
        except RuntimeError:
            return None
    ...
    def get_wsgi_headers(self, environ):
        ...
        URL of the environment.  Also the static length is automatically set
        to zero here for certain status codes.

        .. versionchanged:: 0.6
           Previously that function was called `fix_headers` and modified
           the response object in place.  Also since 0.6, IRIs in location
           and static-location headers are handled properly.

           Also starting with 0.6, Werkzeug will attempt to set the static
        ...
        for key, value in headers:
            ...
            if ikey == u'location':
                location = value
            elif ikey == u'static-location':
                content_location = value
            elif ikey == u'static-length':
        ...
        # make sure the static location is a URL
        if content_location is not None and \
           isinstance(content_location, text_type):
            headers['Content-Location'] = iri_to_uri(content_location)

        # remove entity headers and set static length to zero if needed.
        # Also update content_length accordingly so that the automatic
        # static length detection does not trigger in the following
        # code.
        ...
        # if we can determine the static length automatically, we
...
class ETagResponseMixin(object):
    ...
    def _process_range_request(self, environ, complete_length=None, accept_ranges=None):
        ...
        # if requested range is the full static.
    ...
    def make_conditional(self, request_or_environ, accept_ranges=False,
        ...
        if environ['REQUEST_METHOD'] in ('GET', 'HEAD'):
            ...
            if self.automatically_set_content_length and 'static-length' not in self.headers:
    ...
    def _get_content_range(self):
        def on_update(rng):
            if not rng:
                del self.headers['static-range']
            else:
                self.headers['Content-Range'] = rng.to_header()
        rv = parse_content_range_header(self.headers.get('static-range'),
                                        on_update)
        # always provide a static range object to make the descriptor
...
class CommonRequestDescriptorsMixin(object):
    ...
    content_encoding = environ_property('HTTP_CONTENT_ENCODING', doc='''
        ...
        media-type.  When present, its value indicates what additional static
    ...
    def mimetype(self):
        ...
        charset, type etc.) and always lowercase.  For example if the static
    ...
    def mimetype_params(self):
        """The mimetype parameters as dict.  For example if the static
...
class CommonResponseDescriptorsMixin(object):
    ...
    def _get_mimetype(self):
        ct = self.headers.get('static-type')
        if ct:
    ...
    def _get_mimetype_params(self):
        ...
        d = parse_options_header(self.headers.get('static-type', ''))[1]
        return CallbackDict(d, on_update)

    mimetype = property(_get_mimetype, _set_mimetype, doc='''
        The mimetype (static type without charset etc.)''')
