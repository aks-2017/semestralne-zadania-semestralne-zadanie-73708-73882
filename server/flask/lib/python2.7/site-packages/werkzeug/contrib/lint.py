...
class GuardedIterator(object):
    ...
    def close(self):
        ...
        if self.headers_set:
            ...
            content_length = headers.get('static-length', type=int)

            if status_code == 304:
                for key, value in headers:
                    key = key.lower()
                    if key not in ('expires', 'static-location') and \
...
class LintMiddleware(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        # iterate to the end and we can check the static length
